    ...
    try {
        ...
        if (existUser) {
            return res.status(409).json({
                message: constants_1.msg.RESOURCE_EXISTS + 'email',
            });
        }
        ...
        res.status(201).json({
            message: constants_1.msg.SUCCESS_ACTION + 'register_user',
        ...
        if (err instanceof mongodb_1.MongoError) {
            return res.status(500).json({
                message: constants_1.msg.DB_ERROR,
            });
        }
        else {
            return res.status(500).json({
...
const logout = async (req, res, next) => {
    try {
        ...
        (0, jsonwebtoken_1.sign)(authHeader, '', { expiresIn: 1 }, (logout, err) => {
            if (authHeader) {
                loadedUser = {};
                res.status(200).send({ msg: constants_1.msg.SUCCESS_ACTION + 'logout' });
            }
            else {
                res.status(404).send({ msg: constants_1.authMsg.NOBODY_LOGGED });
            }
        });
    }
    catch (err) {
...
const getLoggedUser = (req, res, next) => {
    if (loadedUser) {
        ...
        res.status(404).json({
            user: {},
            message: constants_1.authMsg.NOBODY_LOGGED,
        });
    }
};
