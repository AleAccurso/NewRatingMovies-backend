...
exports.updateMovie = async (req, res, next) => {
    const movie = movieModel.findOneAndUpdate({ _id: req.params.id }, {
        ...req.body,
    }, (err) => {
        if (err) {
            res.status(500).send({ message: msg.SERVER_ERROR });
...
exports.updateMetaData = async (req, res, next) => {
    ...
    if (format === "x-matroska") {
        console.log(`mkvpropedit "${job.path}" -e info -s title="${job.selectedMovie.title}"`);
        exec(`mkvpropedit "${job.path}" -e info -s title="${job.selectedMovie.title}"`, (error, stdout, stderr) => {
            if (error) {
...
exports.isInDB = async (req, res, next) => {
    const movies = movieModel.findOne({ movieDbId: req.params.movieDBId }, (err, movie) => {
        if (err) {
            res.status(404).send({ message: msg.RESOURCE_NOT_FOUND + "movie" });
        }
        else if (movies) {
